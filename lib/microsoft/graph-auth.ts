...
export function clearGraphAccessTokenCache() {
  cachedToken = null;
}
...
export function requireAzureConfig() {
  ...
  if (!cfg.tenantId || !cfg.clientId || !cfg.clientSecret || !cfg.from) {
    throw new Error(
      "Microsoft Graph not configured. Set AZURE_TENANT_ID, AZURE_CLIENT_ID, AZURE_CLIENT_SECRET, and LOGIN_EMAIL_FROM."
    );
  }
  return cfg;
}
...
export async function getGraphAccessToken(): Promise<string> {
  ...
  if (!res.ok || !data.access_token) {
    throw new Error(
      `Azure token request failed: ${data.error_description ?? data.error ?? "Unknown error"}`
    );
  }
